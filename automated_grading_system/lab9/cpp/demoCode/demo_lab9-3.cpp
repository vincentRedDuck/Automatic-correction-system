...
//--------------- need to rewrite ---------------------
#define LABNUMBER "lab9" //format "lab#", # is number of exercise
#define EXNUMBER "ex9" //format "ex#", # is number of exercise
...
int main(){
   ...
   //if(FILE *file = fopen((charToString(CHECKRESULTDIRPATH) + charToString(LABNUMBER) + charToString("/") + charToString(LABNUMBER) + charToString("-") + charToString(username)).c_str(), "r")){
    ...
    if((pid = fork()) < 0){
        ...
        execlp("diff", "diff", "-a", "-b", "-Z", "-B", (charToString(CHECKRESULTDIRPATH) + charToString(LABNUMBER) + charToString("/") + charToString("true") + charToString(EXERCISENUMBER)).c_str(), studentOutputfile.c_str(), NULL);
