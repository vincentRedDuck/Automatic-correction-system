...
//--------------- need to rewrite ---------------------
#define LABNUMBER "lab0" //format "lab#", # is number of exercise
...
void setEndTime(time_t &due_t){  // score will change 0.7 after the deadline. 
    ...
    timeStruct->tm_sec=0;
    timeStruct->tm_min=20;
    timeStruct->tm_hour =14;
    timeStruct->tm_mday=27;
    timeStruct->tm_mon=9-1;   // -1 is mean that the range is 0 to 11.
...
int main(){
    ...
    string codename = charToString(LABNUMBER) + charToString("-") + charToString(EXERCISENUMBER) + charToString(".cpp");
...
//check it
	...
	if((status >> 8) == 1){
		...
		//cerr << "score: 0.7" << endl;
                execlp("python3","python3", GOOGLESHEETPYTHONPATH, LABNUMBER,"0.7",NULL);
            else
		//cerr << "score: 1.0" << endl;
                execlp("python3","python3", GOOGLESHEETPYTHONPATH, LABNUMBER,"1.0",NULL);
