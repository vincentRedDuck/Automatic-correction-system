...
//--------------- need to rewrite ---------------------
#define LABNUMBER "lab7" //format "lab#", # is number of exercise
#define EXNUMBER "ex7" //format "ex#", # is number of exercise
#define EXERCISENUMBER "3" //format "#", # is number of exercise
...
void setEndTime(time_t &due_t){  // score will change 0.7 after the deadline. 
    ...
    timeStruct->tm_mday=22;
    timeStruct->tm_mon=11-1;   // -1 is mean that the range is 0 to 11.
...
int main(){
   ...
   //if(FILE *file = fopen((charToString(CHECKRESULTDIRPATH) + charToString(LABNUMBER) + charToString("/") + charToString(LABNUMBER) + charToString("-") + charToString(username)).c_str(), "r")){
    ...
    if((pid = fork()) < 0){
        ...
        execlp("diff", "diff", "-b", "-Z", "-B", (charToString(CHECKRESULTDIRPATH) + charToString(LABNUMBER) + charToString("/") + charToString("true") + charToString(EXERCISENUMBER)).c_str(), studentOutputfile.c_str(), NULL);
